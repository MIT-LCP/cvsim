 ...
 */
int numerics(Data_vector *p, Reflex_vector *r, Output_vector *out, Parameter_vector *theta)
{
  ...
  // output array (*out).
  if ((*p).x[1] > tmp_dap && (*p).x[1] < tmp_sap) {
...
int numerics_new_ptr(Data_vector *p, Reflex_vector *r, double hdid, int N_OUT, double *out_vec)
{
  ...
  if (r->step_cnt == 1) {
    ...
    if (p->x[1] > abp_sys) abp_sys = p->x[1];
    if (p->x[1] < abp_dias) abp_dias = p->x[1];
    abp_mean += p->x[1]*hdid;

    // Left ventricular pressure
    if (p->x[0] > lvp_sys) lvp_sys = p->x[0];
    if (p->x[0] < lvp_md) lvp_md = p->x[0];
    if ((r->step_cnt < 200) && (p->dxdt[0] < 20.0))
      lvp_ed = p->x[0];

    // Right ventricular pressure
    if (p->x[3] > rvp_sys) rvp_sys = p->x[3];
    if (p->x[3] < rvp_md) rvp_md = p->x[3];
    if ((r->step_cnt < 200) && (p->dxdt[3] < 20.0))
      rvp_ed = p->x[3];

    // Mean right and left atrial pressures and mean peripheral venous pressure
    right_atrium += p->x[3]*hdid;
    left_atrium  += p->x[0]*hdid;
    periph_vp    += p->x[2]*hdid;

    // Pulmonary artery pressure
    if (p->x[4] > pap_sys) pap_sys = p->x[4];
    if (p->x[4] < pap_dias) pap_dias = p->x[4];
    pap_mean += p->x[4]*hdid;

    // Pulmonary artery pressure
    if (p->x[5] > pvp_max) pvp_max = p->x[5];
    if (p->x[5] < pvp_min) pvp_min = p->x[5];
    pvp_mean += p->x[5]*hdid;
