...
void initial_ptr(Hemo *hemo, Cardiac *cardiac, Micro_r *micro_r, System_parameters *system, Reflex *reflex, Timing *timing)
{
  ...
  (*micro_r).r[0][0] = 3.9;  (*micro_r).r[0][1] = 1.6;
  ...
  (*micro_r).r[3][0] = 8.;  (*micro_r).r[3][1] = 0.5;
  (*micro_r).r[3][2] = 7.8;  (*micro_r).r[3][3] = 10.3;
  ...
  (*reflex)[0].c[1][2] = 0.004;  (*reflex)[0].c[1][3] = 0.010;
...
void mapping_ptr(Hemo *hemo, Cardiac *cardiac, Micro_r *micro_r, System_parameters *system, Reflex *reflex, Timing *timing, Parameter_vector *tmp)
{
  ...
  tmp -> vec[75] =  3825.0;      // Total zero pressure filling volume
  ...
  tmp -> vec[95] =    70.0;      // Maximal external negative pressure
  ...
  tmp -> vec[153] =    2.0;   // TLD art. compliance
  tmp -> vec[154] =  100.0;   // TLD venous compliance
  tmp -> vec[155] =    0.006;  // TLD left ventr. outflow resistance
  tmp -> vec[156] =    1.0;   // TLD peripheral systemic resistance
  tmp -> vec[157] =    0.04;  // TLD venous resistance
  tmp -> vec[158] =   -0.011; // TLD arterial resistance feedback gain
  tmp -> vec[159] =    0.05;  // Cardio-pulmonary resistance feedback gain
  tmp -> vec[160] =   26.5;   // TLD venous tone gain 
  tmp -> vec[161] =  100.00;  // Cardio-pulmonary venous tone gain
  tmp -> vec[162] =   15.00;  // ZPFV of left ventricle
  tmp -> vec[163] =  715.00;  // ZPFV of systemic arteries
  tmp -> vec[164] = 2500.00;  // ZPFV of systemic veins
  tmp -> vec[165] =   15.00;  // ZPFV of right ventricle
  tmp -> vec[166] =   90.00;  // ZPFV of pulmonary arteries
  tmp -> vec[167] =  490.00;  // ZPFV of pulmonary veins
}
